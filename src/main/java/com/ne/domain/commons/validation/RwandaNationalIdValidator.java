package com.ne.domain.commons.validation;
...
public class RwandaNationalIdValidator implements ConstraintValidator<ValidRwandaId, String> {
    ...
    public boolean isValid(String value, ConstraintValidatorContext context) {
        if (value == null || value.isEmpty()) {
            ...
            context.buildConstraintViolationWithTemplate("National Id is required.").addConstraintViolation();
            return false;
        }

        if (value.length() != 16){
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("National Id should be 16 characters in long.").addConstraintViolation();
            return false;
        }
        ...
        if (!matcher.matches()) {
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("Invalid National ID format.").addConstraintViolation();
            return false;
        }
        ...
        if(!isValidSecurityCode(securityCodeDigits)){
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("Invalid Security Code.").addConstraintViolation();
            return false;
        }
